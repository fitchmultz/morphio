...
def align_speakers_to_words(
    ...
    if not words or not diarization.segments:
        logger.warning("No words or segments to align")
        return []
    ...
    logger.info(f"Aligned {len(words)} words into {len(utterances)} utterances")
    return utterances
...
def merge_cross_chunk_speakers(
    ...
    for chunk_idx, (utterances, offset) in enumerate(zip(chunk_utterances, chunk_offsets)):
    ...
    logger.info(
        f"Merged {len(all_segments)} segments into {len(merged_segments)} "
        f"with {speaker_counter} unique speakers"
    )
