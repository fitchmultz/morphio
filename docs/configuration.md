...
| `.env` | Repo-root env file for local dev and Docker (generated via `make env`) |

Only `/.env` and `/.env.example` are allowed in the repo. `/.env` is local-only and must never be tracked by git.
...
| `SECRET_KEY` | **Prod** | generated by `make env` | Strong secret for sessions |
| `JWT_SECRET_KEY` | **Prod** | generated by `make env` | JWT signing key |
...
Run `make env` from the repo root to create/update `/.env` safely:
- Copies `/.env.example` to `/.env` if missing
- Generates strong local values for `SECRET_KEY` and `JWT_SECRET_KEY`

## Production Security Requirements

When `APP_ENV=production`:
1. `SECRET_KEY` must be strong and non-placeholder (`dev_*`, `__GENERATE_SECURE_VALUE__`, `__CHANGE_ME__` are rejected)
2. `JWT_SECRET_KEY` must be strong and non-placeholder (`dev_*`, `__GENERATE_SECURE_VALUE__`, `__CHANGE_ME__` are rejected)
